...
var json_fb='https://taghash.co/mloyalApps/facebookspeciality.php';
var RSS_twitter='https://taghash.co/mloyalApps/twitter.php?uname=speciality';
var RSS_pinterest='http://103.25.128.61/projects/taghash/mloyalApps/pinteresttantra.php?brand=speciality';
var RSS_instagram='https://www.instagram.com/speciality/?__a=1';
var RSS_youtube='http://xceedfbapps.com/mloyalfeeds/youtube.php?brandname=speciality';
var RSS_tumblr='http://xceedfbapps.com/mloyalfeeds/tumblr.php?brandurl=http://speciality.tumblr.com';

var brandname='speciality';
var aboutus_det='speciality';
var brand_url='https://www.speciality.co.in/';
var cart = { cartdata: []};
...
var SERVER = "https://specialityloyalty.mloyalretail.com/m/";
var SERVER1 = "https://specialityloyalty.mloyalretail.com/";
...
$(document).ready(function(event){
			   ...
			   if($.mobile.activePage.is('#homepage')){
                 ...
                 //getContacts();
                  if(localStorage.getItem("usernamespeciality")!=null && localStorage.getItem("usernamespeciality")!='' && localStorage.getItem("passwordspeciality")!=null && localStorage.getItem("passwordspeciality")!='')
                    {
                            $('#username').val(localStorage.getItem("usernamespeciality"));
                            $('#password_myacc').val(localStorage.getItem("passwordspeciality"));
							user.mobile = localStorage.getItem('usernamespeciality');
							var pwd = localStorage.getItem('passwordspeciality');
                        ...
                        if(user.mobile=='undefined' || user.mobile=='null' || user.mobile==null)
                           ...
                           // alert(user.firstname);
                            ...
                            document.getElementById("profilepts1").innerHTML=user.total+' points';
                             ...
                             $("#wait").css("display","none");
                             authenticate();
                          if(imageExists('https://taghash.co/mloyalfileupload/uploads/speciality_'+localStorage.getItem("usernamespeciality")+'.jpg'))
                          {
                              document.getElementById("cardImagemenu").src='https://taghash.co/mloyalfileupload/uploads/speciality_'+localStorage.getItem("usernamespeciality")+'.jpg';
                          }                     
                     }
				   else if(localStorage.getItem("usernamespeciality")!=null && localStorage.getItem("usernamespeciality")!='' && (localStorage.getItem("passwordspeciality")==null || localStorage.getItem("passwordspeciality")=='')) 
	                 {
			...
			success: function(data, textStatus, xhr) {
			 ...
			 if(data.toLowerCase()=='success'){
				 ...
				 $('#login-msg').html('The  passcode has been sent to your mobile.Please check your SMS.');
			      ...
			      $('#username').val(mob);
				  localStorage.setItem('usernamespeciality',mob);
...
function authenticate(){
 ...
 if(uname=='' || pwd==''){
  ...
  $.ajax({
   ...
   localStorage.setItem('usernamespeciality',uname);
   localStorage.setItem('passwordspeciality',pwd);
 ...
 if(user.mobile=='undefined' || user.mobile=='null' || user.mobile==null)
    ...
    {
	    if(localStorage.getItem("profiledone_speciality")){
    ...
    error: function(xhr, textStatus, errorThrown) {
	user.mobile = localStorage.getItem('usernamespeciality');
	pwd = localStorage.getItem('passwordspeciality');
	uname = user.mobile;
    user.balance = localStorage.getItem('balance');
    user.firstname = localStorage.getItem('firstname');
    user.lastname = localStorage.getItem('lastname');
...
$(document).on('pageinit', '#profilepage', function (event, ui) {
	...
	$.ajax({
			...
			success: function(data, textStatus, xhr) {
				...
				localStorage.setItem("profiledone_speciality","Yes");
...
function showRewards(){
	...
	openInWebView('https://specialityloyalty.mloyalretail.com/microsite/');
	//RewardsNA();
	/*
	if(user.mobile==null || user.mobile=='')
	{
...
if(id=='rewardstore' || id=='reward-cat' || id=='reward-brand')
	{
	tabserver = "https://specialityloyalty.mloyalretail.com/Rewards/";
	}
else
	{
	
	tabserver = "https://specialityloyalty.mloyalretail.com/m/";
	}
	$.getJSON(
		tabserver+url, {
		//page: url,
...
function renderTemplate(pageID, json) {
	...
	case 'inbox':
        ...
        str += '<span class="msghdr">DM-SRREST</span><span class="msgTime">'+json[i].msgtime+'</span>';
  ...
  // str += '<a href="javascript:likeMessage_rewards(\''+json.data[i].brand_id+'\',\''+i+'\');" data-role="button"><span id="likingrewards'+i+'"><img src="img/msg_up.png"></span></a>';
   ...
   break;
    ...
    case 'socialwall_youtube':
		...
		//str += '<a href=\"javascript:loadDataGal(\''+gallery_cat+'\');\" data-role=\"button\" data-theme=\"reset\" class=\"ios\"><span><img src=\"img/cheer/speciality.png\" /></span></a></div>';
...
var str = '';
	...
	switch (pageID) {
		...
		case 'aboutus':
			...
			if(parseInt(user.balance)>=0 && parseInt(user.balance)<=25000)
            {
            ...
            else if(parseInt(user.balance)>=50001)
            {
               document.getElementById('silverslab').className = '';
               document.getElementById('goldslab').className = '';
		       document.getElementById('platinumslab').className = '';
  ...
  $.getJSON(
     url, {
         ...
         renderTemplate(id, json);
          ...
          localStorage.setItem('facebook_speciality',JSON.stringify(json.item));
		 ...
		 try{
			 renderTemplate(id,JSON.parse(localStorage.getItem('facebook_speciality')));
		 }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
  ...
  $.getJSON(
     url, {
         ...
         renderTemplate(id, json.feed.entry);
          ...
          localStorage.setItem('youtube_speciality',JSON.stringify(json.feed.entry));
		   ...
		   try{
			 renderTemplate(id,JSON.parse(localStorage.getItem('youtube_speciality')));
		   }
		   catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
...
else if(url==RSS_twitter)
{
  ...
  $.ajax({
            ...
            localStorage.setItem('twitter_speciality', JSON.stringify(data));
                },
                error: function (xhr, textStatus, errorThrown) {
				...
				{
					renderTemplate(id, JSON.parse(localStorage.getItem('twitter_speciality')));
					//alert('here in catch');
				}
  ...
  $.getJSON(
     url, {
         ...
         renderTemplate(id, json.body);
          ...
          localStorage.setItem('pinterest_speciality',JSON.stringify(json.body));
		   ...
		   try{
			    renderTemplate(id,JSON.parse(localStorage.getItem('pinterest_speciality')));
		   }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
...
else if(url==RSS_tumblr)
{
  ...
  $.getJSON(
     url, {
        ...
        //  localStorage.setItem('twitter',JSON.stringify(json.responseData.feed.entries));
       localStorage.setItem('tumblr_speciality',JSON.stringify(json.channel.item));

    

       }).error(function () {
		   try{
			   renderTemplate(id,JSON.parse(localStorage.getItem('tumblr_speciality')));
		   }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
...
else if(url==RSS_instagram)
{
  ...
  $.getJSON(
     url, {
        ...
        //  localStorage.setItem('twitter',JSON.stringify(json.responseData.feed.entries));
       localStorage.setItem('instagram_speciality',JSON.stringify(json.user));

    

       }).error(function () {
		   try{
			   renderTemplate(id,JSON.parse(localStorage.getItem('instagram_speciality')));
		   }catch(err)
		   {
         alert("Error: Could not connect to Server, make sure you are connected to Network");
		   }
...
function receivedEvent(id) {
 ...
 var push = PushNotification.init({ "android": {"senderID": "906780502226"}});
...
function addincart(i)
{
	...
	cart.cartdata.push({ 
   "index":i,
   "url" : "https://specialityloyalty.mloyalretail.com/"+jsonary[i].brand_logo,
...
function checkout(){
  ...
  $.ajax({
    url: 'https://specialityloyalty.mloyalretail.com/Rewards/complete.asp',
    type: 'GET',
    timeout: 50000,
...
function giftfriend(){
  ...
  var val=$('#giftpt').val();
  $.ajax({
    url: 'https://specialityloyalty.mloyalretail.com/m/gift_points_api.asp',
    type: 'GET',
    timeout: 50000,
    dataType: 'text', //xml/html/script/json/jsonp
...
function shareApp()
{
	window.plugins.socialsharing.share('Hi, Join Speciality Preffered.', null, 'https://taghash.co/shareimg/speciality.jpg', 'https://specialityloyalty.mloyalretail.com/mapp');
}
...
function rateApp()
{
	var deviceType = (navigator.userAgent.match(/iPad/i))  == "iPad" ? "iPad" : (navigator.userAgent.match(/iPhone/i))  == "iPhone" ? "iPhone" : (navigator.userAgent.match(/Android/i)) == "Android" ? "Android" : (navigator.userAgent.match(/BlackBerry/i)) == "BlackBerry" ? "BlackBerry" : "null";
		 if (deviceType=='iPad' || deviceType=='iPhone') 
			 ...
			 {
               window.open("market://details?id=com.mobiquest.specialitynew","_system");
...
function loadfbpg()
{
	$.mobile.changePage( "#socialmain_fb", { transition: "none"} ); 
}

function loadinstapg()
{
	$.mobile.changePage( "#socialmain_insta", { transition: "none"} ); 
}

function openfbpage(str)
{
	...
	if(str=='S1')
	{
		...
		openSocial('https://www.facebook.com/MainlandChinaIndia');
		//console.log('11111');
	}
	else if(str=='S2')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/AsiaKitchenbyMainlandChina');
	}
	else if(str=='S3')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/ohcalcuttaindia');
	}
	else if(str=='S4')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/the.riyasat');
	}
	else if(str=='S5')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/SigreeIndia');
	}
	else if(str=='S6')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/barishhofficial');
	}
	else if(str=='S7')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/athoppipola');
	}
	else if(str=='S8')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/HoppipolaKolkata');
	}
	else if(str=='S9')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://www.facebook.com/inside.episode');
	}
	
}

function openinstapage(str)
{
	...
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/mainlandchinaindia');
		//console.log('11111');
	}
	else if(str=='S2')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/asiakitchenindia');
	}
	else if(str=='S3')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/ohcalcuttaindia');
	}
	else if(str=='S4')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/riyasat_india');
	}
	else if(str=='S5')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/sigreeindia');
	}
	else if(str=='S6')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/barishhofficial');
	}
	else if(str=='S7')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/athoppipola');
	}
	else if(str=='S8')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/hoppipola_kol');
	}
	else if(str=='S9')
	{
		//$.mobile.changePage( "#socialpg", { transition: "none"} ); 
		openSocial('https://instagram.com/inside.episode');
	}
...
function showShop()
{
    //openInWebView('https://www.speciality.co.in/');

	var ref = window.open('https://www.speciality.co.in/', '_blank');

	//$.mobile.changePage( "#menupg", { transition: "none"} );
	//var url='https://www.speciality.co.in/';
   ...
   timeout: 50000,
    ...
    success: function(data, textStatus, xhr) {
        ...
        for (var i = 0; i < data.length; i++)
			{
				if(data[i].storename.indexOf('Email Admin')==0 || data[i].storename.indexOf('ONLINE')==0 || data[i].storename.indexOf('online')==0 || data[i].storename.indexOf('test')==0)
				continue;
...
$("input[type=file]").change(function(){
  ...
  // alert($("input[type=file]")[0].files[0]);
	...
	aFormData.append("mobile", localStorage.getItem("usernamespeciality"));
    aFormData.append("brand", "speciality");
	...
	$.ajax(
    {
      ...
      success     : 
        function(data)
        {
         ...
         document.getElementById("cardImagemenu").src='https://taghash.co/mloyalfileupload/uploads/speciality_'+localStorage.getItem("username")+'.jpg';
      //   document.getElementById("cardImage").src='http://103.25.128.61:9296/mloyalfileupload/uploads/speciality_'+localStorage.getItem("username")+'.jpg';
        }
    });
